/**
 * @file tests/integration/dataPenghuni.test.js
...
// 🧩 Mock semua dependency internal (path disesuaikan dari tests/integration)
jest.mock("../../models/userModels");
jest.mock("../../config/database", () => ({
  // prisma mock yang berada di tests/__mocks__/prisma.js
...
describe("📋 Data Penghuni API Test (Jest + Supertest)", () => {
  ...
  describe("POST /pengelola/dataPenghuni/edit", () => {
    test("❌ Gagal jika field wajib tidak diisi", async () => {
      const res = await request(app).post("/pengelola/dataPenghuni/edit").send({
        mahasiswa_id: "",
        nama: "",
        nim: "",
        jurusan: "",
  ...
  describe("POST /pengelola/dataPenghuni/toggle/:mahasiswa_id", () => {
    test("❌ Gagal jika mahasiswa tidak ditemukan", async () => {
      ...
      const res = await request(app).post("/pengelola/dataPenghuni/toggle/999");
  ...
  describe("Error Handling", () => {
    ...
    test("❌ Menangani error database saat edit penghuni", async () => {
      ...
      prisma.mahasiswa.findFirst.mockResolvedValue(null);
      prisma.mahasiswa.update.mockRejectedValue(new Error("Update failed"));
