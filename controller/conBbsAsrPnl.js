...
const getAllBebasAsrama = async (req, res) => {
  try {
    // REPLACED: const data = await BebasAsrama.findAll();
    const data = await prisma.suratbebasasrama.findMany({
      include: {
        mahasiswa: {
          include: {
            user: true
...
const getBuktiPembayaran = async (req, res) => {
  try {
    ...
    // REPLACED: const pengajuan = await BebasAsrama.findByIdWithPembayaran(id);
    const numericId = parseInt(id, 10);
    if (isNaN(numericId)) {
      return res.status(400).json({ success: false, message: "ID tidak valid" });
    }
